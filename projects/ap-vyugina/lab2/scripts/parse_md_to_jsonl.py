...
def main():
    ap = argparse.ArgumentParser(description="Parse Markdown into RAG dataset: split by headings, ignore headings inside code.")
    ...
    for _, file in tqdm(enumerate(files), total=len(files)):
        ...
        rows = build_chunks_for_doc(
            raw=raw_doc,
            md_path=str(Path(file).relative_to(args.input_dir)),
            levels=levels,
...
python scripts/parse_md_to_jsonl.py \
  --input_dir /data/itmo-llm-25/projects/ap-vyugina/lab2/docs \
  --output_jsonl /data/itmo-llm-25/projects/ap-vyugina/lab2/artifacts/md_dataset.jsonl \
  --levels 1,2,3 \
  --chunk_size 1024 \
  --overlap 64 
```
...
```json
{"id":"6f2c2f4f-3f74-4f47-9a56-0a0d2f9c2a4b","heading":"Serving","level":1,"text":"...chunk...","file_path":"getting_started/quickstart.md"}
```
"""
